...
def evaluate_and_draw(model,adv_patch, image_root, gt_root,
    ...
    for img, name, gt in zip(images, image_names, test_gts):
        ...
        if test_type.split("_")[0] == "resize":
            ...
            merge_image = torch.tensor(transforms.Resize([int(h * scale_factor), int(w * scale_factor)])(merge_image))
        merge_image = merge_image.to('cuda:0')
        if model_name == 'DBnet':
            boxes = get_pred_from_boxes(MMOCR(det='DB_r18', recog=None).single_grad_inference(merge_image, feaName=[]))
            ...
            (score_text, score_link, target_ratio) = single_grad_inference(model, merge_image, [], model_name)
...
if __name__=="__main__":
    ...
    for dir in special_eval:
        log_path = 'Mylog/eval_log/{}_{}'.format(model_name,dir)
                ...
                if test_type == "resize":
                    for d1 in os.listdir(temp_gt_path):
                        ...
                        if model_name in ["DBnet", "easyocr"]:
                            save_path = os.path.join(os.path.join("test_save/{}".format(model_name),dir), str(iter), 'test_{}'.format(test_type), d1)
                        else:
                    ...
                    if model_name in ["DBnet","easyocr"]:
                        save_path = os.path.join(os.path.join("test_save/{}".format(model_name), dir), str(iter),
                    ...
                    gt_path = temp_gt_path
                    if model_name in ["DBnet", "easyocr"]:
                        save_path = os.path.join(os.path.join(r"F:\udup\test_save\{}".format(model_name), dir), str(iter), 'original')
                    else:
                    ...
                    logINFO("test_type:{} iter:{} R:{:.4f} P:{:.4f} F:{:.4f} MUI:{:.4f}".format(test_type, iter, R, P, F, pert), log_file)
