...
def get_bit_acc(bits_hat, bits):
    if 'cuda' in str(bits_hat.device):
        bits_hat = bits_hat.cpu()
        bits = bits.cpu()
    bits_hat = torch.round(bits_hat)
    correct_bit = torch.sum(bits_hat == bits, dim=1)  # 维度1上求和
    bit_acc = torch.sum(correct_bit).numpy() / bits.numel()
    return bit_acc
...
def save_adv_patch_img(img_tensor, path):
    img_cv = tensor_to_cv2(img_tensor)
...
def Diverse_module_1(image,now_ti,gap):
    high_index=1.08  # 5次方1.47
    low_index = 0.95  # 5次方0.77
    max_resize_range = (0.80, 1.5)
    # high_index = 1.1  # 5次方1.61
    # low_index = 0.91  # 5次方0.62
    # max_resize_range = (0.6, 2)
    pow_num=now_ti//gap
...
def Diverse_module_2(image,UAU,now_ti,gap):
    ...
    # return resize_image,resize_UAU


def Diverse_module_adv(image,x_d1,UAU,now_ti,gap):
    from watermarklab.noiselayers.diffdistortions import GaussianNoise, GaussianBlur
    ...
    resize_image=Diverse_module_1(image,now_ti,gap)
    # bright_image=brightness(resize_image, x_d1, now_ti)
    # contrast_image=contrast(bright_image, x_d1, now_ti)
    # saturation_image=saturation(contrast_image, x_d1, now_ti)
    # hue_image=hue(saturation_image, x_d1, now_ti)
    noise_image=gaussianNoise(resize_image, x_d1, now_ti)
    blur_image=gaussianBlur(noise_image, x_d1, now_ti)
    # jpeg_image=jpeg(noise_image, x_d1, now_ti)
    # perspective_image=perspective(jpeg_image, x_d1, now_ti)
    h,w=blur_image.shape[2:]
    resize_UAU=transforms.Resize([h, w])(UAU)
    return blur_image,resize_UAU
...
def modified_model_inference(model,img,model_name,is_eval):
    if model_name=='DBnet':
        ...
        # EasyOCR 使用内部的 CRAFT detector，需要处理 DataParallel wrapper
        detector = model.detector
        craft_model = detector.module if hasattr(detector, 'module') else detector
        score_text = get_CRAFT_pred(craft_model, img=img, square_size=1280, is_eval=is_eval)
    return score_text

def single_grad_inference(model,img ,feaName,model_name,is_eval):
    """
    通用的梯度推理函数，支持 CRAFT 和 EasyOCR
    
    Args:
        model: 模型实例 (CRAFT 或 EasyOCR Reader)
        img: 输入图像张量
        feaName: 需要提取的中间层名称列表
        model_name: 模型名称 ('CRAFT' 或 'EasyOCR')
        is_eval: 是否为评估模式
    
    Returns:
        pp_result: 模型预测结果
        fea_hooks: 如果 feaName 非空，返回中间层特征的 hook 列表
    """
    # 对于 EasyOCR，需要获取内部的 CRAFT detector
    if model_name == 'EasyOCR':
        detector = model.detector
        actual_model = detector.module if hasattr(detector, 'module') else detector
    else:
        actual_model = model
    
    # 注册 hook 以提取中间层特征
    if feaName and len(feaName) != 0:
        fea_hooks = get_feas_by_hook(actual_model, hook_name=feaName)
