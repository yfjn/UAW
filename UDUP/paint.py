import torch
from Tools.ImageIO import tensor_to_cv2,img_read
...
for key,iters in iter_dict.items():
    for iter,xname in zip(iters,xnames):
        patch_path="result_save/size={}_step=3_eps=90_lambdaw=2.5/advtorch/advpatch_{}".format(str(key),str(iter))
        img_path='AllData/test/019.png'
        save_path="temp-save"
        ...
        import cv2
        import os
        cv2.imwrite(os.path.join(save_path,"{}-{}.png".format(str(key),str(xname))),tensor_to_cv2(merge_img))
