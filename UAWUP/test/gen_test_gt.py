...
config = "/home/dongli911/anaconda3/envs/udup/lib/python3.8/site-packages/mmocr/configs/textdet/dbnet/dbnet_r18_fpnc_1200e_icdar2015.py"
...
def test_save_easyocr_gt(all_images=all_images,all_abs_images=all_abs_images):
    reader = easyocr.Reader(['en'], gpu=True, model_storage_directory='AllConfig/all_model')
...
def test_save_craft_gt(all_images_ijcai=all_images,all_abs_images_ijcai=all_abs_images):
    ...
    for imgname, item in zip(all_images_ijcai, all_abs_images_ijcai):
        img=img_read(item)
        img=img.cuda()
        (score_text, score_link, target_ratio) = single_grad_inference(model, img, [], model_name='craft',is_eval=True)
...
def test_db():
    img = cv2.imread(img_test_1)
    ocr = MMOCR(det='DB_r18', det_config=config, recog=None)
    boxes = get_pred_boxes_formmocr(ocr.readtext(img))
    print(type(boxes))
    print(boxes)

def test_psenet():
    img = cv2.imread(img_test_1)
    ocr = MMOCR(det='PS_IC15', det_config=config, recog=None)
    result=ocr.readtext(img)
    boxes = get_pred_boxes_formmocr(result)
    recover_mmocr_boxes(img,boxes,model_name="psenet")
    print(boxes)
def test_panet():
    img = cv2.imread(img_test_2)
    ocr = MMOCR(det='PANet_IC15', det_config=config, recog=None)
    result=ocr.readtext(img)
    boxes = get_pred_boxes_formmocr(result)
