...
def test_save_easyocr_scale_gt(all_images=all_images,all_abs_images=all_abs_images):
    ...
    for scales in resize_scales:
        ...
        for imgname, item in zip(all_images, all_abs_images):
            ...
            resize_cvimg = tensor_to_cv2(transforms.Resize([int(h * scales), int(w * scales)])(img_torch))
...
def test_craft():
    img = img_read(img_test_1)
    model = load_CRAFTmodel()
    (score_text, score_link, target_ratio) = single_grad_inference(model, img, [], model_name='craft')
    boxes = get_CRAFT_box(score_text, score_link, target_ratio, text_threshold=0.7, link_threshold=0.4, low_text=0.4)
    print(boxes)
