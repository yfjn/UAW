...
if __name__ == "__main__":
    ...
    for color in color_eval:
        print("color:", color)
        awti_dir = 'eval_watermark/color/awti_{}'.format(color)
        ...
        logINFO(f"{color} 平均恢复前比特正确率: {np.mean(acc_before_recovers) * 100:.2f}%", log_file, file=True, console=True)
        logINFO(f"{color} 平均恢复后比特正确率: {np.mean(acc_after_recovers) * 100:.2f}%", log_file, file=True, console=True)
